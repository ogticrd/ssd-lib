...
const COUNTER_TIMEOUT = 2000;

const counters = document.getElementsByClassName("animated-counter");

for (let counter of counters) {
  const value = parseFloat(counter.innerText);
  const durationEach = COUNTER_TIMEOUT / value;
  animateCounter(counter, value, durationEach);
}

function animateCounter(counter, value, duration) {
  let count = 0;
...
for (let progressBar of progressBars) {
  // Get the value of the progress bar via the data attribute
...
/****************
 *
 * De aquí en adelante es de Tomás Familia
 *
 */
...
calendarPickers = document.querySelectorAll("input.date");

calendarPickers.forEach(function (e) {
  const datepicker = new Datepicker(e, {
    // ...options
  });
});

const elem = document.querySelectorAll("input.date");
...
for (i = 0; i < l; i++) {
  ...
  a.setAttribute("class", "form-select");
...
function closeAllSelect(elmnt) {
  ...
  y = document.getElementsByClassName("form-select");
  xl = x.length;
  yl = y.length;
  for (i = 0; i < yl; i++) {
    if (elmnt == y[i]) {
